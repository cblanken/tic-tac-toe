...
export function buildGameTree(
  ...
  if (
    checkForWinner(root.boardState, minimizer.symbol) ||
  ...
  root.boardState.forEach((row, rowIndex) => {
    row.forEach((cell, colIndex) => {
      if (root.boardState[rowIndex][colIndex].value === EMPTY) {
        ...
        newBoard[rowIndex][colIndex] = isMaximizer
          ? maximizer.symbol
          : minimizer.symbol;
        ...
        root.children.push(
          buildGameTree(newNode, minimizer, maximizer, !isMaximizer, depth - 1)
        );
      }
    });
  });
